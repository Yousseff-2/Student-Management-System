...
public class ActiveStudentAdminController {
    ...
    private void loadStudent() {
        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
            String sql = "SELECT * FROM Accounts WHERE Role = 'Student' AND IsActive = 1";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    studentList.add(new Student(id, name, username, major, age));
                }
                    ...
                    try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
                        ...
                        try (PreparedStatement checkUsernameStmt = conn.prepareStatement(checkUsernameSql)) {
                            ...
                            if (count > 0) {
                                ...
                                String sql = "INSERT INTO Accounts (Name, Username, Major, Age, Role, IsActive, Password) VALUES (?, ?, ?, ?, 'Student', 1, ?)";
                                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                                    ps.setString(1, name);
